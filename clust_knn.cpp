...
void clust_knn::init(GLUT3d * _my3d,
					           GLUT2d * _my2d,
	...
	dat.init(nj, N);
	origIndex.init(nj); badData.init(nj);
  ...
  int ix, iy, iz, ind;
	...
	for(j=0; j<nj; j++){
		...
		if((j%dispfact)==0){
			printf("%s\n(%s%d%s/%s%d%s)%s<==>%s(%s%d%s/%s100%s)%s", KGRN, KRED, j+1, KMAG, KRED, nj, KGRN, KMAG, KGRN, KRED,
             (int)(100.*((float)(j+1))/((float)nj)), KMAG, KRED, KGRN, KNRM);
		}
...
int clust_knn::get_n_knn_centres(){ // the number of classes.
